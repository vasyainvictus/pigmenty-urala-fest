# routes/main.py - ФИНАЛЬНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ
...
# Импортируем стандартные инструменты Flask-Login
from flask_login import login_required, current_user

main_bp = Blueprint('main', __name__)

# Мы больше не используем ваш самодельный декоратор, но я оставлю его закомментированным на всякий случай
# def login_required(f):
#     ...

@main_bp.route('/dashboard')
@login_required # Используем стандартный декоратор
def dashboard():
    # === НАЧАЛО ИСПРАВЛЕНИЙ ===
    # 1. Мы получаем пользователя ТОЛЬКО через current_user.
    #    Декоратор @login_required гарантирует, что к этому моменту
    #    current_user уже существует и он валиден.
    user = current_user

    # 2. МЫ ПОЛНОСТЬЮ УДАЛИЛИ СТАРЫЙ БЛОК ПРОВЕРКИ:
    #    if not user:
    #        session.clear()
    #        return redirect(url_for('auth.login'))
    #    Именно он был ПРИЧИНОЙ ЦИКЛА. Он здесь больше не нужен.
    
    role = user.role
    # === КОНЕЦ ИСПРАВЛЕНИЙ ===


    # ▼▼▼ ВСЯ ВАША ОРИГИНАЛЬНАЯ ЛОГИКА ОСТАЕТСЯ ЗДЕСЬ БЕЗ ИЗМЕНЕНИЙ ▼▼▼
    ...
    if role == 'participant':
        ...
        return render_template('dashboard/dashboard_participant.html',
                               user=user,
        ...
        return render_template('dashboard/dashboard_judge.html',
                               user=user,
                               schedule_items=schedule_items,                                                 
                               highlight_slot_ids=highlight_slot_ids,
                               now=datetime.now())
...
# ▼▼▼ ВАШИ ОСТАЛЬНЫЕ МАРШРУТЫ ОСТАЮТСЯ ЗДЕСЬ БЕЗ ИЗМЕНЕНИЙ ▼▼▼
...
def my_judging():
    ...
    if assigned_slot_ids:
        all_assigned_contests = TimeSlot.query.filter(
            TimeSlot.id.in_(assigned_slot_ids), TimeSlot.type == 'judging'
        ).options(
            joinedload(TimeSlot.participants),
            joinedload(TimeSlot.nomination_template).joinedload(NominationTemplate.criteria),
            joinedload(TimeSlot.day)
        ...
        for contest in all_assigned_contests:
            criteria_count = len(contest.nomination_template.criteria) if contest.nomination_template and contest.nomination_template.criteria else 0
            participants_count = len(contest.participants)
            if participants_count == 0 or criteria_count == 0:
                pending_contests.append(contest)
                continue
            scores_count_by_judge = Score.query.filter(Score.judge_id == user.id, Score.participation_id.in_([p.id for p in contest.participants])).count()
            total_expected_scores = participants_count * criteria_count
            if scores_count_by_judge >= total_expected_scores:
                judged_contests.append(contest)
            else:
